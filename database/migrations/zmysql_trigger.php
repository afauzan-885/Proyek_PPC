...
return new class extends Migration
{
    ...
    //sinkronisasi kalkulasi data harga material dengan qty di PO Masuk
        DB::unprepared('
        CREATE TRIGGER `Sinkronisasi_dan_Kalkulasi_PO_Masuk` AFTER UPDATE ON `pb__finish_goods`
        FOR EACH ROW 
        BEGIN
            UPDATE po__po_masuk
    ...
    //Sinkronisasi dan Kalkulasi data Pembelian Material
        DB::unprepared('
        CREATE TRIGGER `Sinkronisasi_Data_Pembelian_Material` AFTER UPDATE ON `pb__warehouses`
        FOR EACH ROW 
        BEGIN
